...
- (void)setUpWaves{
    ...
    //第二个波浪颜色
    self.secondWavesColor = [UIColor colorWithRed:253/255.0 green:131/255.0 blue:81/255.0 alpha:1.0];
   // self.secondWavesColor = [UIColor orangeColor];
    //设置波浪的速度
    _wavesSpeed = 1/M_PI;
...
//        self.secondWavesLayer.fillColor = [UIColor colorWithRed:253/255.0 green:131/255.0 blue:81/255.0 alpha:1.0].CGColor;
...
-(void)setCurrentFirstWaveLayerPath{
    ...
    CGFloat y = currentK;
    //将点移动到 x=0,y=currentK的位置  左加右减
    CGPathMoveToPoint(path, nil, 0, y);
    
    for (NSInteger i =0.0f; i<=WavesWidth; i++) {
        //余弦函数波浪公式
        y = _waveA * cos(_waveW*i- offsetX)+currentK-_waveA;
        
        //如果需要正弦函数的峰顶和余弦函数的峰底对应,可以替换成下方公式均可
       // y = _waveA * cos(_waveW*i + offsetX+M_PI_2)+currentK-_waveA;
